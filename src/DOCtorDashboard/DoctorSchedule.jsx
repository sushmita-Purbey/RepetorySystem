...
const DoctorSchedule = () => {
  ...
  const doctorId = "doc123"; // Make sure this matches your MongoDB document ID

  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        console.log('Fetching schedule for doctorId:', doctorId);
        const response = await fetch(`http://localhost:5000/api/schedule/${doctorId}`, {
          method: 'GET',
          headers: {
      ...
      <div className="absolute top-[100px] shadow border right-[300px] left-[500px] max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-lg">
        ...
        {savedSchedule && (
          <div className="mt-6 p-4 bg-gray-100 rounded">
            ...
            <p><strong>Time Slots:</strong> {savedSchedule.timeSlots.map(slot => `${slot.start} - ${slot.end}`).join(", ")}</p>
